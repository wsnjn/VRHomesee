 ...
 * 文件名称：RoomInfoRepository.java
 * 开发者：牛迦楠
 * 专业：软件工程（中外合作办学）
 * 学校：东华理工大学
 * 功能描述：房屋信息数据访问接口，提供多维度的房源检索（行政区划、价格、面积、租赁类型等）、区域去重统计及房东侧房源管理功能
 * 创建日期：2026-01-06
 */
...
public interface RoomInfoRepository extends JpaRepository<RoomInfo, Long>, JpaSpecificationExecutor<RoomInfo> {
        ...
        List<String> findDistinctDistrictsByProvinceAndCity(@Param("province") String province,
        ...
        // 根据省份、城市和小区名称获取房源
        List<RoomInfo> findByProvinceAndCityAndCommunityNameAndStatusOrderByCreatedTimeDesc(String province,
                        String city,
