...
public interface TenantManagementRepository extends JpaRepository<TenantManagement, Long> {
     ...
     * @return 日期和数量列表
     */
    @Query("SELECT DATE(t.contractSignedTime) as date, COUNT(t) FROM TenantManagement t WHERE t.contractSignedTime >= :date GROUP BY DATE(t.contractSignedTime)")
    List<Object[]> countByContractSignedTimeAfter(@Param("date") java.time.LocalDateTime date);
}
